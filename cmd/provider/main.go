...
func main() {
	var (
		app              = kingpin.New(filepath.Base(os.Args[0]), "Terraform based Crossplane provider for EquinixMetal").DefaultEnvars()
	...
	kingpin.FatalIfError(apis.AddToScheme(mgr.GetScheme()), "Cannot add EquinixMetal APIs to scheme")
	kingpin.FatalIfError(controller.Setup(mgr, log, rl, setup, ws, pconfig.GetProvider(tf.Provider()), 1), "Cannot setup EquinixMetal controllers")
